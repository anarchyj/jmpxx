...
`result` transport for one chain instantiates many more templates than the nine for the
hand-written baseline, and takes roughly twice as long to translate. How many more is the standard
library's answer as much as this library's, and it varies by more than a factor of three across the
compilers this project tests, so what is stated here is the bound rather than a figure: the
deterministic compile-cost gate holds the count under a fixed budget on every build. The cost over
a plain integer return is the cost of the type-level guarantees.
